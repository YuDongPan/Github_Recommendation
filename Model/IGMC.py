...
class GNN(nn.Module):
    ...
    def __init__(self, dataset, gconv=GCNConv, latent_dim=[32, 32, 32, 1],
        ...
        self.lin1 = nn.Linear(sum(latent_dim), 128)
        if self.regression:
            self.lin2 = nn.Linear(128, 1)
        else:
            self.lin2 = nn.Linear(128, dataset.num_classes)
...
class DGCNN(GNN):
    ...
    def __init__(self, dataset, gconv=GCNConv, latent_dim=[32, 32, 32, 1], k=30,
        ...
        if k < 1:  # transform percentile to number
            ...
            k = max(10, k)  # k is no smaller than 10
        self.k = int(k)
...
class DGCNN_RS(DGCNN):
    ...
    def __init__(self, dataset, gconv=RGCNConv, latent_dim=[32, 32, 32, 1], k=30,
        ...
        super(DGCNN_RS, self).__init__(dataset, GCNConv, latent_dim, k, regression,
                                       adj_dropout=adj_dropout, force_undirected=force_undirected)
        self.convs = torch.nn.ModuleList()
